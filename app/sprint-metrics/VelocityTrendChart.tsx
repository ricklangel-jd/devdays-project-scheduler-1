 ...
 * for y = m*x + b given arrays of x and y values.
 */
...
const VelocityTrendChart = ({ data }: VelocityTrendChartProps) => {
  ...
  // Build per-project velocity series (sorted by offset ascending = oldest first)
  // Exclude the last 2 sprints back — they don't have 3 sprints of history for velocity
  const { projectKeys, projectNames, projectVelocities, sprintLabels } = useMemo(() => {
    // Collect all offsets sorted ascending (e.g. -5, -4, ... -1, 0)
    const allOffsets = data.grids.map((g) => g.offset).sort((a, b) => a - b);

    // Drop the 2 oldest offsets (they lack 3-sprint history for velocity)
    const offsets = allOffsets.length > 2 ? allOffsets.slice(2) : allOffsets;

    // Collect all unique project keys and names (stable order from first grid)
    ...
    // Build sprint labels from offsets (oldest first)
    ...
    // Build velocity per project: for each offset, compute 3-sprint avg
    // Uses all grids (including excluded ones) for lookback data
    const projectVelocities = new Map<string, SprintVelocity[]>();

    for (const pk of projectKeys) {
      const series: SprintVelocity[] = [];

      for (const offset of offsets) {
        let sum = 0;
        let count = 0;
        for (const off of [offset, offset - 1, offset - 2]) {
          const g = data.grids.find((gr) => gr.offset === off);
          const r = g?.rows.find((rr) => rr.projectKey === pk);
          if (r) {
        ...
        const velocity = count > 0 ? Math.round((sum / 3) * 10) / 10 : 0;

        series.push({ offset, sprintName, velocity });
      }

      projectVelocities.set(pk, series);
    }

    return { projectKeys, projectNames, projectVelocities, sprintLabels };
  }, [data]);

  // Max velocity across all projects for Y-axis scaling
  const maxVelocity = useMemo(() => {
    let max = 0;
    for (const series of projectVelocities.values()) {
      for (const s of series) {
        if (s.velocity > max) max = s.velocity;
      }
    }
    return max;
  }, [projectVelocities]);
  ...
  const yScale = useMemo(() => {
    const niceMax = Math.max(10, Math.ceil(maxVelocity * 1.15 / 10) * 10);
    ...
    <Box sx={{ flex: 1, overflow: 'auto', p: 1 }}>
      <Paper ref={containerRef} sx={{ px: 3, py: 1.5, overflow: 'hidden' }} elevation={1}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
          Rolling 3-sprint average of resolved points per project. Dashed lines show the linear trend.
        </Typography>
          ...
          <svg width={svgWidth} height={svgHeight} style={{ display: 'block', margin: '0 auto' }}>
            <g transform={`translate(${MARGIN.left}, ${MARGIN.top})`}>
              ...
              {projectKeys.map((pk, pkIdx) => {
                ...
                const series = projectVelocities.get(pk) ?? [];
