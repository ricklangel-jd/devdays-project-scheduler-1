...
interface PiPlanningChartProps {
  ...
  piLabel: string;
  /** Per-sprint capacity segments (when provided, replaces scalar capacity) */
  capacitySegments?: SprintCapacityInfo[];
}
...
const buildCapacitySegments = (segments: SprintCapacityInfo[]): CapacitySegment[] => {
...
const PiPlanningChart = ({ epics, piLabel, capacitySegments }: PiPlanningChartProps) => {
  ...
  const epicColorMap = useMemo(() => {
    ...
    epics.forEach((epic, idx) => {
      if (!map.has(epic.key)) map.set(epic.key, idx);
    });
    return map;
  }, [epics]);
  ...
  // Build capacity segments
  const builtSegments = useMemo(
    () => capacitySegments !== undefined ? buildCapacitySegments(capacitySegments) : null,
    [capacitySegments]
  );

  const totalCapacity = useMemo(() => {
    if (!builtSegments) return 0;
    return Math.round(builtSegments.reduce((sum, s) => sum + s.capacity, 0) * 10) / 10;
  }, [builtSegments]);

  const showCapacity = builtSegments !== null;

  // Max Y value
  const maxValue = useMemo(
    () => Math.max(totalPoints, showCapacity ? totalCapacity : 0, 10),
    [totalPoints, totalCapacity, showCapacity]
  );
  ...
  // Build stacked demand segments (bottom-up)
  const segments = useMemo(() => {
    ...
    return visibleEpics.map((epic) => {
      ...
      const patternId = epic.isStretch ? `pp-stripe-${epic.key.replace(/[^a-zA-Z0-9]/g, '-')}` : '';
      const fill = epic.isStretch ? `url(#${patternId})` : baseColor;
      stackY -= barHeight;
      return { epic, x: demandBarX, y: stackY, width: BAR_WIDTH, height: barHeight, color: baseColor, fill };
    });
  ...
  const overUnder = showCapacity && totalPoints > 0 ? totalPoints - totalCapacity : null;
    ...
    >
      ...
      <svg width={svgWidth} height={svgHeight} style={{ display: 'block', margin: '0 auto' }}>
        ...
        <g transform={`translate(${MARGIN.left}, ${MARGIN.top})`}>
          ...
          {/* Stacked demand bar */}
          {segments.map((seg) => (
            <Tooltip key={seg.epic.key} title={`${seg.epic.key}: ${seg.epic.summary} — ${seg.epic.piPoints} pts${seg.epic.isStretch ? ' (Stretch)' : ''}`} arrow>
              <rect x={seg.x} y={seg.y} width={seg.width} height={seg.height} fill={seg.fill} stroke={seg.color} strokeWidth={0.5} />
            </Tooltip>
          ))}
          {totalPoints > 0 && (
            <text x={demandBarX + BAR_WIDTH / 2} y={yScale.toY(totalPoints) - 6} textAnchor="middle" fontSize={13} fontWeight="bold" fill="#333">{totalPoints}</text>
          )}

          {/* Capacity bar — stacked per sprint */}
          {showCapacity && builtSegments && (() => {
            let capStackY = chartHeight;
            const topY = chartHeight - (totalCapacity / yScale.max) * chartHeight;
            return (
              <>
                {builtSegments.map((seg, idx) => {
                  ...
                  return (
                    <Tooltip key={idx} title={seg.tooltipLabel} arrow>
                      <rect x={capacityBarX} y={segY} width={BAR_WIDTH} height={segH} fill={seg.color} stroke={seg.strokeColor} strokeWidth={0.5} />
                    </Tooltip>
                  );
                })}
                <text x={capacityBarX + BAR_WIDTH / 2} y={topY - 6} textAnchor="middle" fontSize={13} fontWeight="bold" fill="#333">{totalCapacity}</text>
              </>
            );
          })()}

          {/* X-axis labels */}
          <text x={demandBarX + BAR_WIDTH / 2} y={chartHeight + 18} textAnchor="middle" fontSize={12} fontWeight="bold" fill="#333">
            {showCapacity ? 'Demand' : piLabel}
          </text>
          {showCapacity && (
            <>
              <text x={capacityBarX + BAR_WIDTH / 2} y={chartHeight + 18} textAnchor="middle" fontSize={12} fontWeight="bold" fill="#333">Capacity</text>
              {overUnder !== null && (
                <text x={clusterX + clusterWidth / 2} y={chartHeight + 36} textAnchor="middle" fontSize={11} fontWeight="bold" fill={overUnder > 0 ? '#d32f2f' : '#2e7d32'}>
                  {piLabel} — {overUnder > 0
                    ? `+${Math.round(overUnder * 10) / 10} over`
                    : overUnder < 0
                      ? `${Math.round(Math.abs(overUnder) * 10) / 10} under`
                      : 'Balanced'}
                </text>
              )}
            </>
          ...
          <g transform={`translate(${chartWidth + 20}, 0)`}>
            {epics.map((epic, idx) => {
              ...
              return (
                <g key={epic.key} transform={`translate(0, ${rowY})`} opacity={hasPoints ? 1 : 0.35}>
                  <rect width={14} height={14} fill={hasPoints ? (epic.isStretch ? `url(#${patternId})` : baseColor) : 'none'} stroke={baseColor} strokeWidth={hasPoints ? 0.5 : 1} strokeDasharray={hasPoints ? 'none' : '3,2'} rx={2} />
                  <text x={20} y={11} fontSize={11} fill="#333">{`${epic.key} — ${epic.piPoints} pts${epic.isStretch ? ' (S)' : ''}`}</text>
                </g>
              );
            })}
            {showCapacity && (
              <g transform={`translate(0, ${(epics.length + 1) * LEGEND_ROW_HEIGHT})`}>
                ...
                <text x={20} y={11} fontSize={11} fill="#333">{`Capacity — ${totalCapacity} pts`}</text>
              </g>
            )}
          </g>
        </g>
